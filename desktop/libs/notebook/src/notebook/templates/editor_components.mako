    ...
    <div data-bind="delayedOverflow, css: resultsKlass" style="margin-top: 5px; position: relative;">
      ...
      <table class="table table-compressed">
        ...
        <tbody data-bind="foreach: $parent.history">
          <tr class="pointer" data-bind="click: function() { if (getSelection().toString().length == 0) { $parent.ace().setValue(query); hueUtils.changeURL(url); $('.right-panel').animate({ scrollTop: '0px' }); } }">
